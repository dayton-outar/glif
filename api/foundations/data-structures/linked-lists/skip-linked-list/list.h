...
public:
    ...
    void choosePowers();
    int  chooseLevel();
    T* kListSearch(const T&);
    void kListInsert(const T&);
private:
    typedef kNode<T> *nodePtr;
...
template<class T>
T* kList<T>::kListSearch(const T& key) {
    if (isEmpty())
	return 0;
    nodePtr prev, curr;
...
template<class T>
void kList<T>::kListInsert(const T& key) {
