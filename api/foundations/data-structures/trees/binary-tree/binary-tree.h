...
public:
    ...
    TreeNode(const T& e, TreeNode<T> *l = 0, TreeNode<T> *r = 0) {
        el = e; left = l; right = r; 
    }
    T el;
...
public:
    ...
    void deleteByMerging(TreeNode<T>*&);
...
protected:
    ...
    void recursiveInsert(TreeNode<T>*&, const T&);
    T* search(TreeNode<T>*, const T&) const;
    T* recursiveSearch(TreeNode<T>*, const T&) const;
...
template<class T>
void Tree<T>::clear(TreeNode<T> *p) {
    if (p != 0) {
         clear(p->left);
         clear(p->right);
         delete p;
...
void Tree<T>::insert(const T& el) {   
    ...
    else prev->right = new TreeNode<T>(el);
}

template<class T>
void Tree<T>::recursiveInsert(TreeNode<T>*& p, const T& el) {
    if (p == 0)
         p = new TreeNode<T>(el);
    else if (el < p->el)
...
template<class T>
T* Tree<T>::search(TreeNode<T>* p, const T& el) const {
    while (p != 0)
        if (el == p->el)
             return &p->el;
        else if (el < p->el)
...
template<class T>
T* Tree<T>::recursiveSearch(TreeNode<T>* p, const T& el) const {
    if (p != 0)
         if (el == p->el)
              return &p->el;
         else if (el < p->el)
...
template<class T>
void Tree<T>::deleteByCopying(TreeNode<T>*& node) {    
...
template<class T>
void Tree<T>::findAndDeleteByCopying(const T& el) {    
    TreeNode<T> *p = root, *prev = 0;
     while (p != 0 && !(p->el == el)) {
         prev = p;
         if (el < p->el)
...
template<class T>
void Tree<T>::deleteByMerging(TreeNode<T>*& node) {   
    TreeNode<T> *tmp = node;
    if (node != 0) {
...
template<class T>
void Tree<T>::MorrisPostorder() {
    TreeNode<T> *p = new TreeNode<T>(), *tmp, *q, *r, *s;
    p->left = root;
    while (p != 0)
        if (p->left == 0)
             p = p->right;
