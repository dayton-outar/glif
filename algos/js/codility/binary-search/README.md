...
In a binary search we use the information that all the elements are sorted. Let's try to solve the task in which we ask for the position of value _x_ in a sorted arry ${a_0 \leq a_1 \leq \ldot\leq a_{n - 1} }$. Let's see how the number of candidates is reduced, for example for the value ${x = 31}$.
...
In many tasks, we should return some integer